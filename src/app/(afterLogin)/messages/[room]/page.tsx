...
export default function ChatRoom() {
  ...
  const messages = [
    {messageId: 1, roomId: 123, id: 'hoinkim',  content: '안녕하세요.', createdAt: new Date()},
  ...
  return (
    <main className={style.main}>
      ...
      <div className={style.list}>
        {messages.map((m) => {
          if (m.id === 'hoinkim') { // 내 메시지면
            return (
              <div
                key={m.messageId}
                className={cx(style.message, style.myMessage)}>
                <div className={style.content}>{m.content}</div>
                <div className={style.date}>{dayjs(m.createdAt).format('YYYY년 MM월 DD일 A HH:mm')}</div>
              </div>
            );
          }
          return (
